#ifndef ROBOTRAINER_MODALITIES_PATH_TRACKING_CONTROLLER_H
#define ROBOTRAINER_MODALITIES_PATH_TRACKING_CONTROLLER_H

#include "robotrainer_modalities/modalities_controller_base.h"
#include "robotrainer_modalities/ModalitiesPathTrackingControllerParameters.h"
#include "robotrainer_modalities/ModalitiesPathTrackingControllerConfig.h"
...
{
	template <typename T> class ModalitiesPathTrackingController : public robotrainer_modalities::ModalitiesControllerBase<T>{
		public:
				ModalitiesPathTrackingController();
    ...
    protected:
			...
			bool use_modalities_path_tracking_controller_;
			...
			tf2::Vector3 vel_limit_factor_vec_;
			double eps_;
...
// 			bool humanEndangered(const robotrainer_helper_types::wrench_twist& hf_mv_msg, const robotrainer_helper_types::wrench_twist& vf_mv_msg);
			...
			tf2::Vector3 calcVelLimitationFactorPerDim(const tf2::Vector3& human_vel_lin, const tf2::Vector3& modalities_vel_lin);
			double scaleVelFactorBack(const double& vel_limit_factor_goal);
			tf2::Vector3 scaleVelFactorBackVec(const tf2::Vector3& vel_limit_factor_goal);
// 			geometry_msgs::Twist limitVirtualForces(const robotrainer_helper_types::wrench_twist& hf_mv_msg, const robotrainer_helper_types::wrench_twist& vf_mv_msg);
		...
		private:
			...
			robotrainer_modalities::ModalitiesPathTrackingControllerParameters params_;
			dynamic_reconfigure::Server<robotrainer_modalities::ModalitiesPathTrackingControllerConfig> dynamic_reconfigure_server_;
			
			//FUNCTIONS
			void reconfigureRequest(const robotrainer_modalities::ModalitiesPathTrackingControllerConfig& config, uint32_t level);
