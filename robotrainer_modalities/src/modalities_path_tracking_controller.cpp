#include "robotrainer_modalities/modalities_path_tracking_controller.h"
...
template <typename T> ModalitiesPathTrackingController<T>::ModalitiesPathTrackingController() : params_{ros::NodeHandle("/modalities/modalitiesPathTrackingController")}, dynamic_reconfigure_server_{ros::NodeHandle("/modalities/modalitiesPathTrackingController")}
{
					//initialize the defaultvalues s
					params_.fromParamServer();
					
					// Dynamic reconfigure
					dynamic_reconfigure_server_.setCallback(boost::bind(&ModalitiesPathTrackingController<T>::reconfigureRequest, this, _1, _2));
			 ...
			 pluginlib::ClassLoader<ModalityBase<geometry_msgs::Twist>> vf_loader_ptr_("robotrainer_modalities", "robotrainer_modalities::ModalityBase<geometry_msgs::Twist>");
				...
				test_pub_ = nh_.advertise<geometry_msgs::Vector3>("modalities/modalitiesPathTrackingController/test", 1000);
				HF_HV_Energy_pub_ = nh_.advertise<std_msgs::Float64>("modalities/modalitiesPathTrackingController/hf_vh_energy", 1000);
				VF_MV_Energy_pub_ = nh_.advertise<std_msgs::Float64>("modalities/modalitiesPathTrackingController/hf_mh_energy", 1000);
}


template <typename T> bool ModalitiesPathTrackingController<T>::update(const T& data_in, T& data_out)
{
// 	ROS_INFO("[modalities_path_tracking_controller.cpp] update");
	if(this->modalitie_configured_){
		data_out = data_in;
		
		vf_modalitie_ptr_->update(data_in.twist_, data_out.twist_);

		if(use_modalities_path_tracking_controller_){
// 		ROS_INFO("[modalities_path_tracking_controller.cpp] use = true");
...
// 		}
		...
		ROS_WARN_ONCE("[modalities_path_tracking_controller.cpp] modalities not configured");
	}
		
}


template <typename T> bool  ModalitiesPathTrackingController<T>::configure()
{
...
template <typename T> tf2::Vector3 ModalitiesPathTrackingController<T>::getLastVelocity()
{
	return vf_modalitie_ptr_->getLastVelocity();
}
			
template <typename T> tf2::Vector3 ModalitiesPathTrackingController<T>::getLastscaledForce()
{ 
	return vf_modalitie_ptr_->getLastscaledForce();
}

template <typename T> bool ModalitiesPathTrackingController<T>::checkHumanLetRobotGo(const tf2::Vector3& human_input_force)
{
...
template <typename T> bool ModalitiesPathTrackingController<T>::humanEndangeredVelocity()
{
			//get Integralvalues
			double hf_mv_val = integral_HF_MV_ptr_->getIntegralValue();
			double vf_mv_val = integral_VF_MV_ptr_->getIntegralValue();
// 			ROS_INFO("[modalities_path_tracking_controller.cpp] hf_mv_val: [%.2f]|| vf_mv_val: [%.2f]", hf_mv_val, vf_mv_val);
...
// 				ROS_INFO("[modalities_path_tracking_controller.cpp] ANGLE: [%.2f]", angle);
...
template <typename T> geometry_msgs::Twist ModalitiesPathTrackingController<T>::limitModalitiesVelocity(geometry_msgs::Twist input_twist)
{
		...
		ROS_INFO("[modalities_path_tracking_controller.cpp] LimitFactor:[%.2f]", limitation_factor);
...
template <typename T> double ModalitiesPathTrackingController<T>::calcVelLimitationFactor(const tf2::Vector3& human_vel_lin, const tf2::Vector3& modalities_vel_lin)
{
	return (human_vel_lin.length() <= modalities_vel_lin.length()) ? (human_vel_lin.length() / modalities_vel_lin.length() ) : 1.0;
}

template <typename T> tf2::Vector3 ModalitiesPathTrackingController<T>::calcVelLimitationFactorPerDim(const tf2::Vector3& human_vel_lin, const tf2::Vector3& modalities_vel_lin)
{
...
template <typename T> double ModalitiesPathTrackingController<T>::scaleVelFactorBack(const double& vel_limit_factor_goal)
{
...
template <typename T> tf2::Vector3 ModalitiesPathTrackingController<T>::scaleVelFactorBackVec(const tf2::Vector3& vel_limit_factor_goal)
{
...
template <typename T> void ModalitiesPathTrackingController<T>::reconfigureRequest(const robotrainer_modalities::ModalitiesPathTrackingControllerConfig& config, uint32_t level)
{
	params_.fromConfig(config);
	
	use_modalities_path_tracking_controller_ = params_.use_modalities_path_tracking_controller;
...
PLUGINLIB_EXPORT_CLASS(robotrainer_modalities::ModalitiesPathTrackingController<robotrainer_helper_types::wrench_twist>, robotrainer_modalities::ModalitiesControllerBase<robotrainer_helper_types::wrench_twist>)
