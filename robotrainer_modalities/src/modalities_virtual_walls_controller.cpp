...
template <typename T> bool ModalitiesVirtualWallsController<T>::humanEndangeredVelocity()
{
			...
			ROS_DEBUG("[modalities_virtual_walls_controller.cpp] hf_mv_val: [%.2f]|| vf_mv_val: [%.2f]", hf_mv_val, vf_mv_val);
			...
			if(	hf_mv_val < 0.0 && vf_mv_val > 0.0){
				...
				ROS_DEBUG("[modalities_virtual_walls_controller.cpp] ANGLE: [%.2f]", angle);
